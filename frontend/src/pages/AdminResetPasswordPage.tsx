...
import { adminResetPassword, getAdminCsrfToken } from '../utils/api';
...
const AdminResetPasswordPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await adminResetPassword(email, password, csrfToken);
  ...
  if (!email) {
    return <div className="flex items-center justify-center h-screen">No email provided for password reset.</div>;
  }
      ...
      <div className="w-full max-w-md">
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 sm:p-8 border border-gray-200 dark:border-gray-700">
          ...
          <div className="text-center mb-8">
            <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Admin Reset Password</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {success && (
              <div className="p-3 text-sm text-green-600 bg-green-50 dark:bg-green-900/20 dark:text-green-400 rounded-lg border border-green-200 dark:border-green-700">
                {success}
              </div>
            )}
            <div>
